 ...
 * 具体功能可以查看文档
 */
public class SCTBot extends JcqAppAbstract implements ICQVer, IMsg, IRequest {

    public static long SCT = 719423012L;
    ...
    public static void main(String[] args) {
        ...
        // 要测试主类就先实例化一个主类对象
        SCTBot SCTBot = new SCTBot();
        // 下面对主类进行各方法测试,按照JCQ运行过程，模拟实际情况
        SCTBot.startup();// 程序运行开始 调用应用初始化方法
        SCTBot.enable();// 程序初始化完成后，启用应用，让应用正常工作

        // 依次类推，可以根据实际情况修改参数，和方法测试效果
        // 以下是收尾触发函数
        // demo.disable();// 实际过程中程序结束不会触发disable，只有用户关闭了此插件才会触发
        SCTBot.exit();// 最后程序运行结束，调用exit方法
    }
    ...
    public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg,
        ...
        CommandHandler.getInstance().active(fromGroup, msg, true);
